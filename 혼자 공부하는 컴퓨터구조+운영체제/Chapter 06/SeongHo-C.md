...
- 캐시 메모리[:link:](#캐시-메모리)
  - 저장 장치 계층 구조[:link:](#저장-장치-계층-구조)
  - 캐시 메모리[:link:](#캐시-메모리)
  - 참조 지역성 원리[:link:](#참조-지역성-원리)
  </details>
...
<img width="400" alt="image" src="https://github.com/SeongHo-C/reading-books-for-programmers/assets/83394485/6eb98803-7a6b-4445-bb3c-c0bfcbee6f6c">
...
- 메모리에서 CPU가 사용할 일부 데이터를 미리 캐시 메모리로 가지고 와서 활용함으로써 CPU의 연산 속도와 메모리 접근 속도의 차이를 조금이나마 줄일 수 있다.
- CPU(코어)와 가장 가까운 캐시 메모리를 `L(level)1 캐시`, 그다음 가까운 캐시 메모리를 `L2 캐시`, 그다음 가까운 캐시 메모리를 `L3 캐시`라고 부른다.
  <img width="400" alt="image" src="https://github.com/SeongHo-C/reading-books-for-programmers/assets/83394485/476e9910-5956-4efc-a5d8-7c41c369ac6e">
...
- 자주 사용될 것으로 예측한 데이터가 실제로 들어맞아 캐시 메모리 내 데이터가 CPU에서 활용될 경우를 `캐시 히트`라고 한다. 반대로 예측이 틀려 메모리에서 필요한 데이터를 직접 가져와야 하는 경우를 `캐시 미스`라고 한다.
- 캐시 메모리는 한 가지 원칙에 따라 메모리로부터 가져올 데이터를 결정한다. 바로 `참조 지역성의 원리`이다. 이는 CPU가 메모리에 접근할 때의 주된 경향을 바탕으로 만들어진 원리이다.
